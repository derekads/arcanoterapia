// src/components/orb/liquidOrbShader.ts

/**
 * ORBE LÍQUIDO — shader WGSL
 *
 * Transcrito sem alterações do arquivo enviado. Mantido inteiro de propósito:
 * todos os presets (9 a 23) continuam disponíveis, então trocar o estilo é
 * mexer em um número do `uniform`, não reescrever o shader.
 *
 * Os pontos de entrada `ribbon_*` (estilo 24) ficam no fonte mas não são
 * usados: aquele preset exige um segundo alvo de renderização e 221 mil
 * instâncias, e o app roda no estilo 21. Se um dia quisermos o estilo 24,
 * basta criar os pipelines correspondentes em LiquidOrb.tsx.
 */
