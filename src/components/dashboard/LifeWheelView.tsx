...
const SATISFACTION_COLOR = (n: number) =>
    n <= 3 ? 'text-rose-400' : n <= 6 ? 'text-amber-400' : 'text-emerald-400';
...
function RadarChart({ currentData, prevData, goalData, pulsing }: {
    ...
    return (
        <svg viewBox="0 0 300 300" className="w-full h-full overflow-visible drop-shadow-[0_0_30px_rgba(245,158,11,0.1)]">
            {/* Grid */}
            {[2, 4, 6, 8, 10].map(r => (
            ...
            {/* Goal layer */}
            <polygon points={poly(goalPts)} fill="rgba(245,158,11,0.05)" stroke="#f59e0b" strokeWidth="1.5" strokeDasharray="5 4" strokeOpacity="0.5" />
            ...
            />
            {currentPts.map((p, i) => (
                <motion.circle key={i} layout cx={p.x} cy={p.y} r="4" fill="#f59e0b" />
            ))}
            {goalPts.map((p, i) => (
                <circle key={`g${i}`} cx={p.x} cy={p.y} r="3" fill="none" stroke="#f59e0b" strokeWidth="1.5" opacity="0.6" />
            ))}
            ...
            {AREAS.map((area, i) => {
                ...
                return (
                    <text key={area} x={cx + r * Math.cos(angle)} y={cy + r * Math.sin(angle)}
                        ...
                        fill={pulsing ? 'rgba(245,158,11,0.6)' : 'rgba(255,255,255,0.35)'}
...
// ─── ONBOARDING WIZARD ────────────────────────────────────────────────────────
function OnboardingWizard({ arcano, onComplete, progress, updateLifeArea, getLifeAreaRating, getCurrentYearAndQuarter }: any) {
            ...
            <div className="w-full max-w-2xl">
                ...
                <AnimatePresence mode="wait">
                    <motion.div key={step} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                        <div className="mb-8">
                            <p className="text-[10px] uppercase tracking-[0.3em] text-amber-500/60 mb-2">Passo {step + 1} de 3</p>
                            ...
                            <p className="text-amber-500/50 text-[11px] mt-2 italic">{steps[step].hint}</p>
                        </div>
                                        ...
                                        <div key={area} className="bg-slate-900/50 border border-white/5 rounded-2xl p-4">
                                            <div className="flex items-center justify-between mb-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-amber-500 font-mono font-bold text-sm w-5 text-right">{nota}</span>
                                                </div>
                                            </div>
                                            <input type="range" min="0" max="10" value={nota}
                                                onChange={e => updateLifeArea(area, +e.target.value, undefined, { ano, trimestre: currentQ })}
                                                className="w-full h-1.5 appearance-none bg-white/10 rounded-full outline-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:bg-amber-500 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-[0_0_12px_rgba(245,158,11,0.8)]"
                                                style={{ background: `linear-gradient(to right,#f59e0b 0%,#f59e0b ${nota * 10}%,rgba(255,255,255,0.05) ${nota * 10}%)` }}
                                            />
                                        </div>
                                    );
                                })}
                                <div className="text-center text-[11px] text-slate-500 mt-2">
                                    {filledCount < 3 ? `Avalie mais ${3 - filledCount} área(s) para continuar` : `✓ ${filledCount} áreas avaliadas`}
                                </div>
                            </div>
                        )}
                                    ...
                                    return (
                                        <button key={area} onClick={() => togglePriority(area)}
                                            className={cn(
                                                ...
                                                isPriority ? 'bg-amber-500/20 border-amber-500/60 shadow-[0_0_20px_rgba(245,158,11,0.2)]' : 'bg-slate-900/50 border-white/5 hover:border-white/20'
                                            )}>
                                            <div className="flex items-center gap-2 mb-2">
                                                <Icon size={16} className={isPriority ? 'text-amber-500' : 'text-slate-400'} />
                                                <span className={cn('text-sm font-serif', isPriority ? 'text-amber-100' : 'text-slate-300')}>{AREA_LABELS[area]}</span>
                                                {isOraculo && <span className="ml-auto text-[9px] bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded font-bold">Oráculo</span>}
                                            </div>
                                            <div className="text-xs text-slate-500">Atual: <span className="font-mono text-amber-500">{nota}</span>/10</div>
                                        </button>
                                    );
                                })}
                            </div>
                        ...
                        {step === 2 && (
                            <div className="space-y-4">
                                <div className="bg-slate-900/60 border border-amber-500/10 rounded-2xl p-6">
                                    <p className="text-[10px] uppercase tracking-widest text-amber-500/60 mb-3 font-bold">Foco Escolhido: {selectedPriorities.map(a => AREA_LABELS[a]).join(' + ') || 'Não definido'}</p>
                                    ...
                                    <textarea rows={4}
                                        ...
                                        className="w-full bg-slate-950/80 border border-white/5 rounded-xl p-4 text-sm text-slate-200 placeholder:text-slate-700 focus:border-amber-500/30 transition-all outline-none resize-none"
                                    />
                                    ...
                                    <div className="mt-4 grid grid-cols-3 gap-2">
                                        {['Específica', 'Mensurável', 'Arcana'].map(label => (
                                            <div key={label} className="text-center p-2 bg-white/[0.02] rounded-lg border border-white/5">
                                                <p className="text-[9px] uppercase tracking-widest text-amber-500/50 font-bold">{label}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                ...
                <div className="flex gap-3 mt-8">
                    ...
                    <button
                        ...
                        className="flex-1 py-4 rounded-2xl bg-gradient-to-r from-amber-600 to-amber-500 text-black font-bold text-sm tracking-widest uppercase shadow-[0_0_30px_rgba(245,158,11,0.3)] hover:shadow-[0_0_40px_rgba(245,158,11,0.5)] transition-all disabled:opacity-40 disabled:cursor-not-allowed">
...
function AreaCard({ area, arcano, rating, updateLifeArea, updateLifeAreaGoal, toggleSubMeta, toggleMicroAcao, period, prevNota, allRatings }: any) {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            className="group bg-slate-900/40 hover:bg-slate-900/60 rounded-[2.5rem] border border-white/5 hover:border-amber-500/20 transition-all overflow-hidden">
            <div className="p-8">
                {/* Header */}
                <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-amber-500/10 flex items-center justify-center border border-amber-500/20 group-hover:border-amber-500/40 transition-colors">
                            <Icon size={26} className="text-amber-500 drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]" />
                        </div>
                        <div>
                    ...
                    <div className="text-right">
                        <span className="text-4xl font-mono font-light text-amber-500/90">{nota}</span>
                        {notaAlvo !== nota && <div className="text-[9px] text-amber-500/40 font-bold uppercase tracking-widest">Alvo: {notaAlvo}</div>}
                    </div>
                </div>
                ...
                <div className="relative mb-6">
                    <input type="range" min="0" max="10" value={nota}
                        ...
                        className="w-full h-1.5 appearance-none bg-white/10 rounded-full outline-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:bg-amber-500 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-[0_0_15px_rgba(245,158,11,0.8)]"
                        style={{ background: `linear-gradient(to right,#f59e0b 0%,#f59e0b ${nota * 10}%,rgba(255,255,255,0.05) ${nota * 10}%)` }}
                    />
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setShowGoalForm(!showGoalForm)} className="text-[10px] text-amber-500/60 hover:text-amber-500 transition-colors">
                            {showGoalForm ? 'Fechar' : metaGoal ? 'Editar' : '+ Definir'}
                        </button>
                    </div>

                    {metaGoal && !showGoalForm && (
                        <p className="text-[12px] text-slate-300 bg-white/[0.02] p-4 rounded-2xl border border-amber-500/10 italic">"{metaGoal}"</p>
                    )}
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <p className="text-[11px] text-white/70 leading-relaxed">
                                <span className="text-amber-500/80 font-bold uppercase mr-1">Visão do Arcano:</span>
                                {(() => {
                                ...
                                if (interpretation?.check_diario) {
                                    return (
                                        <div className="mt-3 pt-3 border-t border-white/5 flex items-center gap-2">
                                            ...
                                            <p className="text-[10px] text-amber-500/60 font-medium italic">
                            ...
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                                <div className="space-y-3 bg-slate-900/60 rounded-2xl p-4 border border-white/5">
                                    <textarea rows={3} value={goalDraft} onChange={e => setGoalDraft(e.target.value)}
                                        ...
                                        className="w-full bg-slate-950 border border-white/5 rounded-xl p-3 text-xs text-slate-200 placeholder:text-slate-700 focus:border-amber-500/30 outline-none resize-none" />
                                    <div className="flex items-center gap-3">
                                        ...
                                        <input type="number" min="1" max="10" value={targetDraft} onChange={e => setTargetDraft(+e.target.value)}
                                            className="w-16 bg-slate-950 border border-white/10 rounded-lg p-2 text-sm text-amber-500 text-center outline-none focus:border-amber-500/40" />
                                        <span className="text-[10px] text-slate-600">/10 até fim do trimestre</span>
                                    </div>
                                    <button onClick={saveGoal} className="w-full py-2 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 rounded-xl text-amber-400 text-[11px] font-bold uppercase tracking-widest transition-all">
                                        Salvar Objetivo
                                    </button>
                                </div>
                            </motion.div>
                        ...
                        <div className="space-y-2">
                            {subMetas.map(sm => (
                                <button key={sm.id} onClick={() => toggleSubMeta(area, sm.id, period)}
                                    ...
                                    <div className={cn('w-4 h-4 rounded flex items-center justify-center border transition-all flex-shrink-0', sm.done ? 'bg-emerald-500 border-emerald-500' : 'border-white/20 group-hover/sm:border-amber-500/40')}>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {microAcoes.slice(0, 3).map(ma => (
                                <button key={ma.id} onClick={() => toggleMicroAcao(area, ma.id, period)}
                                    className={cn('text-[10px] px-3 py-1.5 rounded-full border transition-all font-medium', ma.done ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400 line-through' : 'bg-white/5 border-white/10 text-slate-400 hover:border-amber-500/30 hover:text-amber-400')}>
                                    {ma.texto}
                                </button>
                            ))}
                        </div>
                    ...
                    {/* Insight */}
                    <div className="space-y-2">
                        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-amber-500/60">
                            <Sparkles size={11} className="text-amber-500" /> Insight Sazonal
                        </div>
                            ...
                            <div className="flex items-center gap-1 text-[9px] font-mono text-slate-600">
                                {timeline.map((t, i) => (
                                    <React.Fragment key={t.q}>
                                        <span className={cn(t.q === QUARTERS[QUARTERS.findIndex(q => q.id === t.q)].id ? 'text-amber-500/60' : '')}>{t.q}:{t.val}</span>
...
export const LifeWheelView: React.FC<Props> = ({ arcano, onClose, embedded }) => {
    ...
    return (
        <div className={cn('min-h-screen bg-[#020617] text-white', embedded ? '' : 'p-4 md:p-8')}>
            {/* Header */}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm flex items-center gap-2">
                            <Calendar size={14} className="text-amber-500" /> Ciclo {currentYear} — <span className="text-amber-500/80 font-medium">{arcano.nome}</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    {showCheckinAlert && (
                        <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/30 rounded-full animate-pulse">
                            ...
                            <span className="text-[11px] text-amber-400 font-bold">Ritual de Check-in</span>
                        </div>
                    )}
                    {/* Quarter selector */}
                        ...
                        {QUARTERS.map(q => {
                            ...
                            return (
                                <button key={q.id} disabled={future} onClick={() => setSelectedQuarter(q.id)}
                                    ...
                                    {active && <motion.div layoutId="q-pill" className="absolute inset-0 rounded-full border border-amber-500/30" />}
                                    <span className={cn('text-xs font-bold tracking-widest uppercase', active ? 'text-amber-500' : 'text-slate-400')}>{q.label}</span>
                                    <span className="text-[8px] uppercase tracking-widest text-slate-500">{q.months}</span>
                                    {q.id === currentQuarter && !active && <div className="absolute top-1 right-2 w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />}
                ...
                {!bannerDismissed && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-xs font-bold text-amber-500/80 uppercase tracking-widest">{qCopy.title}</p>
                ...
                <div className="lg:col-span-5 lg:sticky lg:top-24 flex flex-col items-center">
                    ...
                    <div className="flex flex-col gap-3 w-full max-w-xs mt-4">
                        <div className="flex gap-3 text-[9px] text-slate-600 font-bold uppercase tracking-widest justify-center">
                            <span className="flex items-center gap-1"><span className="w-3 border-t-2 border-amber-500 inline-block" /> Atual</span>
                            <span className="flex items-center gap-1"><span className="w-3 border-t-2 border-amber-500/40 border-dashed inline-block" /> Meta</span>
